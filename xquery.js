...
(function (win, doc) {
    ...
    Xuery.fn = Xuery.prototype = {
        ...
        init: function (selector, context) {
            ...
            this.length = 0;
            // 默认获取元素的上下文 document
            context = context || documnet;
            // id 选择符，则 按位非 将-1转化为0
            if (~selector.indexOf("#")) {
                ...
                if (typeof arg[0] === "string") {
                    ...
                    for (var key in arg[0]) {
                        for (var index = this.length - 1; index >= 0; index--) {
                            this[index].style[Xuery.camelCase(key)] = arg[0][key];
                        }
                    }
                }
            } else if (len === 2) {
                for (var index = this.length - 1; index >= 0; index--) {
                    this[index].style[Xuery.camelCase(arg[0])] = arg[1];
                }
            }
            return this;
        },
                ...
                if (typeof arg[0] === "string") {
                    ...
                    for (var key in arg[0]) {
                        for (var i = this.length - 1; i >= 0; i++) {
                            this[i].setAttribute(key, arg[0][key]);
                        }
                    }
                }
            } else if (len === 2) {
                for (var i = this.length - 1; i >= 0; i--) {
                    this[i].setAttribute(arg[0], arg[i]);
                }
            }
            return this;
        },
